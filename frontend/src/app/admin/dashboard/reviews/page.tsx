...
export default function ReviewsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-[#F5F0E8]">Avis clients</h2>
        <p className="text-sm text-[#A09A8E] mt-1">{reviews.length} avis</p>
      </div>

      <div className="grid gap-4">
        {reviews.map((review) => (
          <div key={review.id} className="bg-[#1A1A1A] border border-[rgba(245,240,232,0.08)] rounded-xl p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="font-bold text-[#F5F0E8]">{review.customer_name}</p>
                <div className="flex items-center gap-1 mt-1">
                  {[...Array(5)].map((_, i) => (
                    <Icon
                      ...
                      className={i < review.rating ? 'text-[#E8A020]' : 'text-[#5A5550]'}
                    />
                  ))}
                </div>
              </div>
            ...
            {review.title && <p className="text-sm font-bold text-[#F5F0E8] mb-2">{review.title}</p>}
            <p className="text-sm text-[#A09A8E] mb-4">{review.comment}</p>
