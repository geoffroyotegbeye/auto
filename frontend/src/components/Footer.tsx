...
export default function Footer() {
  return (
    <footer className="border-t border-[rgba(245,240,232,0.08)] bg-[#0D0D0D]">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-16">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          {/* Links */}
          <div className="flex flex-wrap items-center justify-center gap-6 text-[11px] font-semibold uppercase tracking-[0.2em] text-[#5A5550]">
          ...
          <div className="flex items-center gap-6">
            {socialLinks.map((s) => (
              <a
                ...
                href={s.href}
                aria-label={s.label}
                className="text-[#5A5550] hover:text-[#E8A020] transition-colors"
              >
                <Icon name={s.icon as Parameters<typeof Icon>[0]["name"]} size={18} />
              </a>
            ))}
            <span className="text-[11px] text-[#5A5550] font-medium ml-2">
              © 2026 Mig Motor
            </span>
          </div>
        </div>
